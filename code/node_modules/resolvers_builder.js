...
const connection = require('datasources/sqlite')
const { getTableByModel } = require('definitions_builder')


class ResolversBuilder {

    constructor() {
        env.logger.setDebugLevels("resolvers") // If subDebugLevel is found
        ...
        env.logger.debug_precise(-616, resolvers)
    ...
    append_relational_resolvers(resolvers) {
        // This attempts to add a resolver entry for every model created in def_builder.
        // All models are used for accessing individual data entries (rows)
        // This logic is for resolving models that can be found within other models.
...
function many_relational_query(resolvers, table_name, model_name, supported_table_name) {
    resolvers[model_name][table_name] = (parent_data, _, __) => { // begin resolver method logic
        if (parent_data) { // parent_data is result from parent model resolver
            ...
            let origin_table = getTableByModel(model_name)
            let origin_id_name = `${origin_table}_id`
            // Fetch target results using current associative table
            return relational_query(table_name, origin_id_name, parent_data.dataValues.id, supported_table_name)
        }
        let conn = new_connection()
...
function direct_query(resolvers, model_name, table_name) {
    resolvers[model_name][table_name] = (parent_data, _, __) => { // begin resolver method logic
        id = parent_data.dataValues[`${table_name}_id`]
        env.logger.debug(2, "Simple Query:", table_name)
        let conn = new_connection()
...
function relational_query(table_name, key, value, supported_table) {
    ...
    */
    ...
    query[key] = value
    let conn = new_connection()
    env.logger.debug(1, "Complex Query:", table_name, query)
    env.logger.debug(2, `Supporting ${supported_table} with data from ${table_name}`)
...
// THIS WORKS (but still needs to handle associative tables)
// create(defs) {
    // query_resolvers = {}
    // def_name = Object.keys(defs.structures)
    // def_name.forEach((def_name, _) => {
    //     if(!(defs.exclusions.includes(def_name))) {
    //         query_resolvers[def_name] = (args, _, __) => {
    //             conn = new connection(databaseInstance)
    //             if (args)
    //                 return conn._run_complex_query(def_name, args)
    //             return conn.findAll(def_name);
    //         }
