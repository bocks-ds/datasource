...
class QueryDefinitionString {
    ...
    set_key_string(name) {
        ...
        */
        let columns = parser.table_structures[name]
        let column_names = Object.keys(columns)
        LOGGER.verbose(`Available columns to add as keys for ${name}: ${column_names}`)
        let output = '('
        ...
        output = output + ')'
        if (output.length > 2) {
            LOGGER.verbose(`Created query keys for '${name}' definition.`)
            return output
        }
        return ''
    }
    ...
    append_column_keys(column, value, output) {
        ...
        */
        LOGGER.debug(`Appending '${value}' to ${column} keys`)
