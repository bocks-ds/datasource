...
class ModelDefinitionString {
    constructor(model_name) {
        this.table_name = helpers.getTableByModel(model_name, LOGGER) // Don't let associative tables pre-write bad info
        this.defString = this.build_def(model_name, parser.table_structures[this.table_name])
        LOGGER.verbose(`Created definition [${this.table_name}] with model [${model_name}]`)
    }
    ...
    build_def(name, data) {
        LOGGER.debug(`Building ${name} definition...`)
        let def = `type ${name} {\n`
    ...
    append_types(def, columns, name) {
        let column_names = []
        try {
            column_names = Object.keys(columns)
        } catch {
            throw new env.DatasourceError(`No columns found for '${name}'`)
        }
        column_names.forEach((column) => {
            let output = this.gql_type(column, columns[column], name)
            LOGGER.debug(`Appending to ${name}: '${output}'`)
            def = `${def}  ${output}\n`
        })
    ...
    gql_id_type(column, parent) {
        ...
        if (column.includes('_id')) {
            ...
            if (name.includes('parent')) {
                ...
                LOGGER.debug(`Setting '${column}' to reference parent model: ${parent_model}`)
    ...
    check_relational_tables(name, def) {
        ...
        if (this.table_name in env.supportedTables) {
            ...
            relational_sources.forEach((source) => {
                ...
                let type = env.associativeTables[source].relationshipType
