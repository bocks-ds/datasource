...
class ModelDefinitionString {
    constructor(model_name) {
        let table = helpers.getTableByModel(model_name) // Don't let associative tables pre-write bad info
        env.logger.debug(0, model_name, '|', table)
        this.defString = this.build_def(model_name, parser.table_structures[table])
    }
    ...
    gql_id_type(column, parent) {
        ...
        // let obj_name = helpers.getModelName(parent) || ref
    ...
    check_relational_tables(name, def) {
        ...
        let table_name = helpers.getTableByModel(name)
