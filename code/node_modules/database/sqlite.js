...
class SqliteDatasource extends DataSource {
    ...
    _get_like(table, args) {
        env.logger.debug(1, "Querying", table, "like", JSON.stringify(args))        
        let query = {}
        Object.keys(args).forEach((arg) => {
            let values = args[arg].split(' ').join(',').split(',') // split on spaces and commas
            let and_query = []
        ...
        return this.models[table].findAll({where: query})
    }
