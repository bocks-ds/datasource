...
const LOGGER = newLogger('parser')

const typeMap = {'string': 'String', 'number': 'Int'}

class ParserABC {
    constructor() {
        LOGGER.verbose(`Loading data from ${this.constructor.name}...`)

        this.table_names = {}
        this.set_data()

        LOGGER.verbose(`Finished loading data from ${this.constructor.name}.`)
    }
    ...
    getFilepaths() {
        ...
        LOGGER.debug(`Reading file group ${dataPath}/*.${env.dataType}...`)
    ...
    read_file(filepath) {
        ...
        LOGGER.debug(`Finished reading file: ${filepath}`)
        return data
      }
    ...
    set_table_structures() {
        ...
        this.table_names.forEach((table_name, _) => {
            LOGGER.debug(`Identifying table structure: ${table_name}`)
    ...
    get_column_types(row, retry=false) {
        ...
        table_keys.forEach((column, _) => {
            ...
            if (types[column] != 'skipped') {
                LOGGER.debug(`Identified column '${column}' as ${types[column]}`)
            }
        })
        return types
    }
