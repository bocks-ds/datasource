...
class ManyToManyResolver {
    constructor(log_id, resolvers, junction_table_name, model_name, supported_table) {
        ...
        */
        let target_table = ''
        try {
            target_table = getTableByModel(model_name)
        } catch (error) {
        ...
        resolvers[model_name][junction_table_name] = (parent_data, _, __) => { // begin resolver method logic
                ...
                env.logger.debug(2, `[${log_id}]`, "Many to Many Query to", junction_table_name)
    ...
    constructor(log_id, resolvers, model_name, table_name, parent=false) {
        ...
        */
        ...
        resolvers[model_name][resolver_name] = (parent_data, _, __) => { // begin resolver method logic
            ...
            env.logger.debug(2, `[${log_id}]`, 'Many to One Query to', resolver_name, 'for ID:', id)
        ...
        return conn._run_complex_query(table_name, query, log_id).then(response => {
            ...
            if(table_name in env.associativeTables) {
                ...
                if (supportTable.relationshipType == 'ManyToMany') {
                    joins_to = getTableByModel(supportTable.model_name)
                    env.logger.debug(2, `Junction table found for ${table_name}: ${joins_to}`)
