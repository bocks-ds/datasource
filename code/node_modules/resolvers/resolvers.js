...
LOGGER = new LoggerInstance('resolvers')

class ManyToManyResolver {
    constructor(log_id, resolvers, junction_table_name, model_name, supported_table) {
        ...
        */
        ...
        LOGGER.debug(`Building many-to-many resolver from ${model_name} to ${target_table} via ${junction_table_name}`)
        ...
        resolvers[model_name][junction_table_name] = (parent_data, _, __) => { // begin resolver method logic
                ...
                LOGGER.verbose(`[${log_id}] Many to Many Query to ${junction_table_name}`)

                return conn.findAll(junction_table_name, log_id);
        }
        return resolvers
    }
}
...
class ManyToOneResolver {
    constructor(log_id, resolvers, model_name, table_name, parent=false) {
        ...
        */
        ...
        LOGGER.debug(`Creating direct resolver in ${model_name} to ${resolver_name}`)
        ...
        resolvers[model_name][resolver_name] = (parent_data, _, __) => { // begin resolver method logic
            ...
            LOGGER.verbose(`[${log_id}] Many to One Query to ${resolver_name} for ID: ${id}`)
...
class RelationalResolver {
    constructor(log_id, table_name, key, value) {
        ...
        */
        ...
        return conn._run_complex_query(table_name, query, log_id).then(response => {
