...
function query_by_search_args(table_name, args) {
    ...
    string_search = {}
    number_queries = {}
    if (Object.keys(terms.string).length > 0) {
        string_search = conn.get_string_query(terms.string)
    }
    if (Object.keys(terms.number).length > 0) {
        number_queries = {}
        // Object.keys(terms.number).forEach((arg) => {
        //     number_queries[arg] = number_queries[arg] || []
        //     number_queries[arg].push(terms.number[arg])
        // })
        Object.keys(terms.number).forEach((arg) => {
            number_queries[arg] = {[Op.and]: terms.number[arg]}
        })
    }
    query = {...string_search, ...number_queries}
