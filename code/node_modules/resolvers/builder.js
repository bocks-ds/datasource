...
const { newConnection, queryBySearchArgs } = require('helpers')
...
class ResolversBuilder {
    ...
    */
        ...
        def_names.forEach((definition, _) => {
            ...
            env.logger.debug(2, "Building resolver for:", definition)

            query_resolvers[definition] = (_, args, __, ___) => {
                let log_id = env.logger.new_log_id()
                let conn = newConnection()
                if (args) {
                    env.logger.debug(1, `[${log_id}]`, "Arguments found:", JSON.stringify(args))
        ...
        Object.keys(env.supportedTables).forEach((supported_table_name, _) => {
            ...
            env.supportedTables[supported_table_name].forEach((table_name, _) => {
                ...
                if (env.associativeTables[table_name].direct)
                    resolvers = new DirectQueryResolver(log_id, resolvers, model_name, table_name)
                else if (table_name == supported_table_name)
                    resolvers = new DirectQueryResolver(log_id, resolvers, model_name, table_name, true)
                else
                    resolvers = new ManyRelationalResolver(log_id, resolvers, table_name, model_name, supported_table_name)
            })
        })
        return resolvers
    }
