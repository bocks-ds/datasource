...
function isEmptyObject(obj){
    // Low complexity check for empty object
    // Returns
    //    boolean telling whether object is empty
...
function queryBySearchArgs(table_name, args, log_id) {
    ...
    */
    ...
    output = conn._run_complex_query(table_name, query, log_id)
    return output
}
...
function parseSearchTerms(args) {
    ...
    */
    ...
    let op_map = {'_max': Op.lte , '_min': Op.gte, '_equals': Op.eq, '_id': Op.eq}
    ...
    Object.keys(args).forEach((arg) => {
        Object.keys(op_map).forEach((op_term) => {
            if (arg.includes(op_term)) {
              let search_term = arg
              if (!arg.includes('_id'))
                    search_term = arg.replace(op_term, '')
